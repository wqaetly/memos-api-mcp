...
const chatMessageSchema = z.object({
    ...
    content: z.union([z.string(), z.array(z.any())]).describe("Message content. Plain text or structured text/image/file parts."),
...
}).describe("Message accepted by MemOS Add Message (system/user/assistant/tool).");
const filterSchema = z.record(z.any()).describe("Memory filter. Supports logical ('and','or') and comparison ('gt','gte','lt','lte') operators. " +
...
const kbFileSchema = z.object({
    ...
    type: z.enum(["document", "skill"]).optional().describe("File type. 'document' (default) or 'skill' (markdown / zip skill package)."),
    content: z.string().describe("File content. Direct URL (e.g. https://...) or Base64 Data URI (e.g. data:text/markdown;base64,...)")
...
// ============================================================================
server.prompt("memos_mandatory_workflow", "Mandatory 3-step memory workflow enforced on every turn (cloud).", async () => ({
    messages: [{
            role: "user",
            content: {
                type: "text",
...
// ============================================================================
server.tool("add_message", `Save messages and extract memories via MemOS cloud /add/message.
...
Use this for storing NEW information. For corrections use add_feedback.`, {
    user_id: z.union([z.string(), z.array(z.string()).max(20)]).optional().describe("User ID or up to 20 user IDs. Defaults to MEMOS_USER_ID when omitted."),
    agent_id: z.union([z.string(), z.array(z.string())]).optional().describe("Agent ID or Agent ID list associated with the messages."),
    conversation_id: z.string().optional().describe("Conversation ID. Messages with the same conversation_id are treated as one context."),
    messages: z.array(chatMessageSchema).min(1).describe("Messages to store. Total tokens must not exceed 40k."),
    app_id: z.string().optional().describe("Associated application identifier"),
    allow_memory_view: z.array(memoryViewSchema).optional().describe("Memory types allowed to be extracted. Defaults to all supported types."),
    tags: z.array(z.string()).optional().describe("Custom topic/classification labels. Filterable via search_memory.filter."),
    info: z.record(z.any()).optional().describe("Custom metadata. Keys become directly filterable in search."),
    allow_public: z.boolean().optional().describe("Allow memory writes to the project public library. Default: false."),
    allow_knowledgebase_ids: z.array(z.string()).optional().describe("Knowledge base IDs allowed for memory writes."),
    async_mode: z.boolean().optional().describe("Enable asynchronous memory processing. Default: true.")
}, async (args) => {
    try {
        const body = {
...
// ============================================================================
server.tool("search_memory", `Retrieve memories via MemOS cloud /search/memory.
...
Use filter for precise scoping by app_id, agent_id, tags, time, or custom info fields.`, {
    ...
    user_id: z.string().optional().describe("User ID to search. Defaults to MEMOS_USER_ID when neither user_id nor agent_id is provided."),
    agent_id: z.string().optional().describe("Agent ID to search. Use instead of user_id when independent Agent memory is enabled."),
    conversation_id: z.string().optional().describe("Conversation ID. Prioritizes memories from the current session for ranking."),
    filter: filterSchema.optional(),
    knowledgebase_ids: z.array(z.string()).optional().describe("Knowledge base IDs accessible to this search. Use ['all'] for every associated knowledge base."),
    include_memory_view: z.array(memoryViewSchema).optional().describe("Memory types allowed in results. Default: ['detail_factual','preference']."),
    memory_limit_number: z.number().int().min(1).max(25).optional().describe("Maximum returned memories after relevance filtering. Default: 9, Max: 25."),
    relativity: z.number().min(0).max(1).optional().describe("Relevance threshold from 0 to 1. Use 0 to disable relevance filtering."),
    include_preference: z.boolean().optional().describe("Deprecated compatibility alias. Prefer include_memory_view."),
    include_tool_memory: z.boolean().optional().describe("Deprecated compatibility alias. Prefer include_memory_view."),
    include_skill: z.boolean().optional().describe("Deprecated compatibility alias. Prefer include_memory_view.")
}, async (args) => {
    try {
        ...
        const legacyViewFlagsPresent = args.include_preference !== undefined ||
        ...
        if (args.include_memory_view === undefined && legacyViewFlagsPresent) {
            const views = ["detail_factual"];
            if (args.include_preference !== false)
                views.push("preference");
            if (args.include_tool_memory === true)
                views.push("tool_memory");
            if (args.include_skill === true)
                views.push("skill");
...
// ============================================================================
server.tool("delete_memory", `Delete memories via MemOS cloud /delete/memory.
...
For user-requested deletion without IDs, first call search_memory to find them.`, {
    memory_ids: z.array(z.string()).optional().describe("Memory IDs to delete (from search_memory or get_user_profile result)."),
    user_id: z.string().optional().describe("Quick delete: remove all memories for this user. Defaults to MEMOS_USER_ID env when present.")
}, async (args) => {
    try {
        const body = {};
        if (args.memory_ids !== undefined)
...
// ============================================================================
server.tool("update_memory", `Directly update an existing memory by ID via MemOS cloud /update/memory.
Use this when the target memory ID is known. At least one of title or content must be supplied.`, {
    memory_id: z.string().describe("Memory ID returned by search_memory, get_user_profile, or another memory listing endpoint."),
    ...
    try {
        ...
        const body = { memory_id: args.memory_id };
...
// ============================================================================
server.tool("add_feedback", `Submit feedback / corrections via MemOS cloud /add/feedback.
Use this for modifying or correcting existing memories, NOT for adding new ones (use add_message).`, {
    ...
    feedback_time: z.string().optional().describe("Structured or natural-language timestamp"),
    allow_public: z.boolean().optional().describe("Allow writing memory to public library. Default: false."),
    allow_knowledgebase_ids: z.array(z.string()).optional().describe("Knowledge base IDs allowed for memory write."),
    user_id: z.string().optional().describe("User ID. Defaults to MEMOS_USER_ID env when omitted.")
}, async (args) => {
    try {
...
// ============================================================================
server.tool("get_user_profile", `Paginated memory listing via MemOS cloud /get/memory.
Use this for identity/profile summaries ("Who am I?") in addition to search_memory.`, {
    page: z.number().int().min(1).optional().describe("Page number. Default: 1."),
    size: z.number().int().min(1).max(50).optional().describe("Items per page. Default: 10, Max: 50."),
    filter: filterSchema.optional(),
    include_preference: z.boolean().optional().describe("Include preference memories. Default: true."),
    include_tool_memory: z.boolean().optional().describe("Include tool memories. Default: true."),
    user_id: z.string().optional().describe("User ID. Defaults to MEMOS_USER_ID env when omitted.")
}, async (args) => {
    try {
        const user_id = args.user_id ?? requireEnv("MEMOS_USER_ID");
...
// ============================================================================
server.tool("extract_memory", `Extract factual / preference memories from a transcript without persisting them.
Useful for previewing what would be stored or for offline analysis.`, {
    messages: z.array(chatMessageSchema).describe("Messages to analyze. Total tokens ≤ 8k."),
    extraction_types: z.array(z.enum(["memory", "preference"])).optional().describe("Which extractions to run. Default: ['memory','preference'].")
}, async (args) => {
    try {
        const body = {
            messages: normalizeMessages(args.messages)
...
// ============================================================================
server.tool("rerank", `Rerank candidate documents by relevance via MemOS reranker model.`, {
    model: z.enum(["memos-reranker-0.6b", "memos-reranker-4b"]).describe("Reranker model. Allowed values: 'memos-reranker-0.6b' (lightweight, faster) or 'memos-reranker-4b' (more accurate)."),
    query: z.string().describe("Query text used for relevance matching."),
    documents: z.array(z.string()).describe("Candidate documents. Total tokens ≤ 8k."),
    top_n: z.number().int().min(1).optional().describe("Return top N results. Default: all.")
}, async (args) => {
    try {
        const body = {
            model: args.model,
...
// Tool: chat  →  POST /chat
// ============================================================================
server.tool("chat", `Chat with memory + knowledge base augmented LLM via MemOS cloud /chat.
Streaming is not supported through this MCP tool; pass stream=false (default).`, {
    ...
    filter: filterSchema.optional(),
    knowledgebase_ids: z.array(z.string()).optional().describe("Knowledge base IDs to consult. Use ['all'] for project-wide access."),
    memory_limit_number: z.number().int().min(1).max(25).optional().describe("Max factual memories. Default: 9, Max: 25."),
    include_preference: z.boolean().optional().describe("Include preference memories. Default: true."),
    preference_limit_number: z.number().int().min(0).max(25).optional().describe("Max preference memories. Default: 9, Max: 25."),
    relativity: z.number().min(0).max(1).optional().describe("Relevance threshold (0–1). Default: 0.45."),
    model_name: z.string().optional().describe("LLM model name. Default: 'qwen2.5-72B-Instruct'."),
    system_prompt: z.string().optional().describe("Custom system instructions"),
    max_tokens: z.number().int().min(1).optional().describe("Max generation tokens. Default: 8192."),
    temperature: z.number().min(0).max(2).optional().describe("Sampling temperature (0–2). Default: 0.7."),
    top_p: z.number().min(0).max(1).optional().describe("Nucleus sampling (0–1). Default: 0.95."),
    user_id: z.string().optional().describe("User ID. Defaults to MEMOS_USER_ID env when omitted.")
}, async (args) => {
    try {
        const user_id = args.user_id ?? requireEnv("MEMOS_USER_ID");
...
// ============================================================================
server.tool("get_message", `Retrieve raw conversation messages via MemOS cloud /get/message.`, {
    conversation_id: z.string().describe("Conversation ID to fetch messages for"),
    message_limit_number: z.number().int().min(1).max(50).optional().describe("Max messages to return. Default: 6, Max: 50."),
    user_id: z.string().optional().describe("User ID. Defaults to MEMOS_USER_ID env when omitted.")
}, async (args) => {
    try {
        const user_id = args.user_id ?? requireEnv("MEMOS_USER_ID");
...
// ============================================================================
server.tool("get_task_status", `Query the status of an async memory task by task_id (returned by add_message in async mode).`, {
...
// ============================================================================
server.tool("create_knowledge_base", `Create a new knowledge base under the current project.`, {
    knowledgebase_name: z.string().describe("Knowledge base name"),
    knowledgebase_description: z.string().optional().describe("Knowledge base description")
}, async (args) => {
    try {
        const body = {
...
// ============================================================================
server.tool("remove_knowledge_base", `Detach a knowledge base from the current project.
...
// ============================================================================
server.tool("add_kb_document", `Upload one or more files (URL or Base64 Data URI) into a knowledge base.
Files are processed asynchronously; check status via get_kb_documents.`, {
    ...
    file: z.array(kbFileSchema).min(1).describe("Files to add. Each entry: { name?, type?='document'|'skill', content (URL or data: URI) }")
}, async (args) => {
    try {
...
// ============================================================================
server.tool("get_kb_documents", `List or fetch knowledge base files via MemOS cloud /get/knowledgebase-file.
Two mutually exclusive modes — pick exactly one:
  ...
  • Mode B (lookup): pass file_ids[]. Do NOT pass knowledgebase_id/type/page/page_size.`, {
    knowledgebase_id: z.string().optional().describe("Mode A only: knowledge base ID to list files from. Mutually exclusive with file_ids."),
    type: z.enum(["document", "skill"]).optional().describe("Mode A only: filter by file type ('document' or 'skill'). Ignored in Mode B."),
    page: z.number().int().min(1).optional().describe("Mode A only: page number. Ignored in Mode B."),
    page_size: z.number().int().min(1).optional().describe("Mode A only: items per page. Ignored in Mode B."),
    file_ids: z.array(z.string()).optional().describe("Mode B only: explicit file IDs to fetch. Mutually exclusive with knowledgebase_id.")
}, async (args) => {
    try {
        const body = {};
        const passthrough = [
...
// ============================================================================
server.tool("delete_kb_document", `Delete one or more knowledge base files (documents or skills) by file_ids.`, {
    file_ids: z.array(z.string()).min(1).describe("File IDs to delete (regular documents or skill files)")
}, async (args) => {
    try {
