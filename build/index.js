...
const chatMessageSchema = z.object({
    ...
    content: z.union([z.string(), z.array(z.any())]).optional().describe("Message content. Plain string or OpenAI-compatible structured parts. Optional for 'assistant' when only tool_calls is present."),
...
}).describe("Chat message following OpenAI ChatCompletion schema (system/user/assistant/tool).");
...
Use this for storing NEW information. For corrections use add_feedback.`, {
    conversation_id: z.string().describe("Conversation ID. Messages with the same conversation_id are treated as one context."),
    messages: z.union([
        z.string(),
        z.array(chatMessageSchema)
    ]).describe("Messages to store. Either a plain string or an array of OpenAI chat messages. Total tokens ≤ 40k."),
    ...
    info: z.record(z.any()).optional().describe("Custom metadata. Keys become filterable in search. " +
        "Example: { app_id, agent_id, source_type, source_url, source_content }"),
    allow_public: z.boolean().optional().describe("Allow memory writes to public library. Default: false."),
    allow_knowledgebase_ids: z.array(z.string()).optional().describe("Knowledge base IDs allowed for memory write. Default: []."),
    async_mode: z.boolean().optional().describe("Enable async memory processing. Default: true."),
...
Use 'filter' for precise scoping (e.g., by tags, app_id, agent_id).`, {
    ...
    knowledgebase_ids: z.array(z.string()).optional().describe("Restrict searchable knowledge bases. Use ['all'] for project-wide access."),
    memory_limit_number: z.number().int().min(1).max(25).optional().describe("Max factual memories. Default: 9, Max: 25. Recommended: 3 for low-noise auto-recall."),
    include_preference: z.boolean().optional().describe("Enable preference memory retrieval. Default: true."),
    preference_limit_number: z.number().int().min(0).max(25).optional().describe("Max preference memories. Default: 9, Max: 25."),
    include_tool_memory: z.boolean().optional().describe("Enable tool memory retrieval. Default: false."),
    tool_memory_limit_number: z.number().int().min(0).max(25).optional().describe("Max tool memories. Default: 6, Max: 25."),
    include_skill: z.boolean().optional().describe("Enable skill/procedure retrieval. Default: false."),
    skill_limit_number: z.number().int().min(0).max(25).optional().describe("Max skill memories. Default: 6, Max: 25."),
    relativity: z.number().min(0).max(1).optional().describe("Relevance threshold (0–1). Default: 0.45. 0 disables filtering."),
    user_id: z.string().optional().describe("User ID. Defaults to MEMOS_USER_ID env when omitted.")
}, async (args) => {
    try {
        const user_id = args.user_id ?? requireEnv("MEMOS_USER_ID");
        const body = { user_id, query: args.query };
        const passthrough = [
